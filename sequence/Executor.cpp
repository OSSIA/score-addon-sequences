...
ProcessExecutorComponent::ProcessExecutorComponent(
    sequence::Model& element, const Execution::Context& ctx,
    const Id<score::Component>& id, QObject* parent)
    : ProcessComponent_T{element, ctx, id, "sequenceExecutorComponent", parent}
{
  auto n = std::make_shared<sequence::node>();
  this->node = n;
   ...
   *         this, [=] (const QColor& c) {
   *
   *   in_exec([c,n=std::dynamic_pointer_cast<sequence::node>(this->node)] {
   *     n->set_color(c);
   *   });
   *
   * });
